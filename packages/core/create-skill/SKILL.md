---
name: create-skill
description: Guide for creating effective skills. Supports global creation in the source repository (~/Workspace/skills-central/packages/custom) and project-level creation (in ./.agents/skills). Use when creating new skills to extend AI capabilities with specialized knowledge, workflows, or tool integrations. 用于创建新技能、构建skill、制作自定义技能等场景。
...
When creating or updating skills in the central source repository (`~/Workspace/skills-central/packages/custom`), default to **cross-platform compatibility**. If any platform-specific behavior is required, isolate it into separate sections or references (or split into platform-specific skills) so other platforms are not affected.
...
5. Create cross-platform symlinks (for central repo skills)
...
For skills in the central repository (`~/Workspace/skills-central/packages/custom`), **always use the script** instead of manual commands:

```bash
# Refresh per-skill links for all supported agents
bash "$HOME/Workspace/skills-central/shared/scripts/install.sh"
```
...
1. Treats `~/Workspace/skills-central` as the authoritative source repository
2. Exports skills to `~/.agents/skills/<skill-name>`
3. Creates/repairs per-skill links under each agent directory (Claude/Codex/Cursor/Gemini/etc.) pointing to `~/.agents/skills/<skill-name>`
...
- **Global skills**: Central repo is `~/Workspace/skills-central`, runtime export dir is `~/.agents/skills`
...
**Note**: Project-level skills are rare. Most skills should be created in the central repository (`~/Workspace/skills-central/packages/custom`) for cross-project reuse.
...
```bash
python3 "$HOME/Workspace/skills-central/packages/core/install-skill/scripts/set_skill_meta.py" \
  --skill-dir "$HOME/Workspace/skills-central/packages/custom/<skill-name>" \
  --usage-zh "<中文用途一句话>" \
