...
删除源码层中的已安装 skill，并同步清理运行时发布结果、已安装列表和 Claude 插件建议文档。
...
按 skill 名删除源码层中的真实目录：

```bash
bash ~/Workspace/skills-central/packages/core/uninstall-skill/uninstall-skill.sh --skill <skill-name>
```
...
```bash
bash ~/Workspace/skills-central/packages/core/uninstall-skill/uninstall-skill.sh --list
```
...
- [`uninstall-skill.sh`](/Users/zhangyufan/Workspace/skills-central/packages/core/uninstall-skill/uninstall-skill.sh)
