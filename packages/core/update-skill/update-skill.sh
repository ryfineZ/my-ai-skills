...
INSTALL_SCRIPT="$REPO_ROOT/packages/core/install-skill/install-skill.sh"
PUBLISH_SCRIPT="$REPO_ROOT/shared/scripts/install.sh"
...
while IFS= read -r row; do
    [[ -n "$row" ]] || continue
    IFS=$'\x1f' read -r _skill_name _skill_path _source _source_type source_repo _source_path _bundle_root _update_group <<< "$row"
...
if [[ "$MODE" == "list" ]]; then
    if [[ "${#ROWS[@]}" -gt 0 ]]; then
        printf '可更新 skill:\n'
        for row in "${ROWS[@]}"; do
            IFS=$'\x1f' read -r skill_name skill_path source source_type source_repo source_path bundle_root update_group <<< "$row"
...
refresh_global_state() {
    if [[ -x "$PUBLISH_SCRIPT" ]]; then
        SKILLS_DIR="$SKILLS_DIR" SOURCE_SKILLS_DIR="$SOURCE_SKILLS_DIR" bash "$PUBLISH_SCRIPT" >/dev/null
    fi
...
collect_group_skill_names() {
    ...
    local row=""
    for row in "${ROWS[@]}"; do
        IFS=$'\x1f' read -r skill_name _skill_path _source _source_type _source_repo _source_path _bundle_root update_group <<< "$row"
...
collect_group_skill_rows() {
    local group="$1"
    local row=""
    for row in "${ROWS[@]}"; do
        IFS=$'\x1f' read -r _skill_name _skill_path _source _source_type _source_repo _source_path _bundle_root update_group <<< "$row"
        if [[ "$update_group" == "$group" ]]; then
            printf '%s\n' "$row"
        fi
    done
...
update_row() {
    local row="$1"
    local skill_name skill_path source source_type source_repo source_path bundle_root update_group
    local before_names=()
    local before_paths=()
    local remote_names=()
    local removed_names=()
    ...
    local item=""
    local group_row=""
    local group_skill_name=""
    local group_skill_path=""
    IFS=$'\x1f' read -r skill_name skill_path source source_type source_repo source_path bundle_root update_group <<< "$row"
    ...
    if [[ "$source_type" == "bundle" ]]; then
        ...
        while IFS= read -r group_row; do
            [[ -n "$group_row" ]] || continue
            IFS=$'\x1f' read -r group_skill_name group_skill_path _rest <<< "$group_row"
            before_names+=("$group_skill_name")
            before_paths+=("$group_skill_path")
        done < <(collect_group_skill_rows "$update_group")
        ...
        SOURCE_SKILLS_DIR="$SOURCE_SKILLS_DIR" RUNTIME_SKILLS_DIR="$SKILLS_DIR" bash "$INSTALL_SCRIPT" "$source_repo" --all-skills --bundle-root "${bundle_root:-skills}" --global
        ...
        if [[ "${#removed_names[@]}" -gt 0 ]]; then
            ...
            if [[ "$PRUNE_MISSING" == true ]]; then
                for idx in "${!before_names[@]}"; do
                    item="${before_names[$idx]}"
                    if contains_skill "$item" "${removed_names[@]}"; then
                        rm -rf "${before_paths[$idx]}"
                    fi
                done
                refresh_global_state
    ...
    SOURCE_SKILLS_DIR="$SOURCE_SKILLS_DIR" RUNTIME_SKILLS_DIR="$SKILLS_DIR" bash "$INSTALL_SCRIPT" "$source_repo" --skill "$skill_name" --global
    UPDATED_COUNT=$((UPDATED_COUNT + 1))
}
...
case "$MODE" in
    skill)
        ...
        matched=0
        for row in "${ROWS[@]}"; do
            IFS=$'\x1f' read -r skill_name _skill_path _rest <<< "$row"
